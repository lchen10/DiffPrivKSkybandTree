...
public class PrivateTopKDomImprove2 {
...
//			debug("Dominating [" + maxTuple.dominatingCount + "]: " + maxTuple);
			...
			// sort all the tuples according to their distance to the top-i result
	...
	//		debug("sorted: " + distTuple);
			...
			// largest distance with top-i result
			DistTuple maxDistTuple = distTuple.get(distTuple.size()-1);
			double maxDist = maxDistTuple.getDist();
			debug("maxDistance: " + maxDist);
...
//			}
			...
			int count = (int) Math.floor(maxDist/0.01);
			double[] probSum = new double[count];
			double d = 0.0;
			...
			//	probSum[n] = probSum[n-1] + (2 * Math.PI * d) * (Math.pow(base, n));
				d += 0.01;
			}
			
			
			
			debug("probSum is: " + probSum[count - 1] + "\n");

			// a random number in [0, 1)
			double randTemp = Math.random();
			debug("rand number is: " + randTemp);

			double rand = randTemp * probSum[count - 1];
			debug("rand: " + rand);
			...
			double radius = idx * 0.01;
			debug("Private radius is: " + radius);
