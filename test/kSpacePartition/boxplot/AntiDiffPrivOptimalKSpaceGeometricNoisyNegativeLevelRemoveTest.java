...
public class AntiDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node.currentID = 1;
				Node root = p.partitionByOptimalKNoisyNegative(k, "dataset", "anti10k.csv", level, true, epslevel,
						eps * (delta - trate), ",", dw, dh, Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("boxplot/noisynegative/anti/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("boxplot/noisynegative/anti/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "boxplot/noisynegative/anti", "boxplot/noisynegative/anti",
	...
	public void testLoadPoints() throws Exception {
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("boxplot/noisynegative/anti-geometric" + "-eps-" + eps + ".csv");
			out.println(
			...
			PrintWriter outdetail = new PrintWriter("boxplot/noisynegative/anti-detail" + "-eps-" + eps + ".csv");
			outdetail.println(
					"k,eps,minsize, kdetal, max level, split rate, tree rate, removal level, true coutn, private count,tp,fp,fn,precision,recall,fmeasure,totalerror,avgerror");
			PrintWriter outbox = new PrintWriter("boxplot/noisynegative/anti-boxplotF1" + "-eps-" + eps + ".csv");
			String delimit = "";
			for (int k : ks) {
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/" + "anti10k.csv" + "-BNL-k-" + k + ".csv";
											String privatefile = "boxplot/noisynegative/anti/"
