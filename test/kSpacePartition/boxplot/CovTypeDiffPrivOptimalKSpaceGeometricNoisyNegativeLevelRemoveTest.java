...
public class CovTypeDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node.currentID = 1;
				Node root = p.partitionByOptimalKNoisyNegative(k, "dataset", "covtype.csv", level, true, epslevel,
						eps * (delta - trate), ",", dw, dh, Comparison.MAX, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("boxplot/noisynegative/covtype/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("boxplot/noisynegative/covtype/dfoptimalKgrid-nglr-k-" + k
						+ "-level-" + level + "-r-" + srate + "-t-" + trate + "-rl-" + removalLevel + "-min-" + minsize
						+ "-kdelta-" + kdelta + "-eps-" + eps + "." + ite + ".csv");
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "boxplot/noisynegative/covtype", "boxplot/noisynegative/covtype",
						new Comparison[] { Comparison.MAX, Comparison.MAX }, delta, null);
	...
	public void testLoadPoints() throws Exception {
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("boxplot/noisynegative/covtype-geometric" + "-eps-" + eps + ".csv");
			out.println(
			...
			PrintWriter outdetail = new PrintWriter("boxplot/noisynegative/covtype-detail" + "-eps-" + eps + ".csv");
			outdetail.println(
					"k,eps,minsize, kdetal, max level, split rate, tree rate, removal level, true coutn, private count,tp,fp,fn,precision,recall,fmeasure,totalerror,avgerror");
			PrintWriter outbox = new PrintWriter("boxplot/noisynegative/covtype-boxplotF1" + "-eps-" + eps + ".csv");
			String delimit = "";
			for (int k : ks) {
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/" + "covtype.csv" + "-BNL-k-" + k + ".csv";
											String privatefile = "boxplot/noisynegative/covtype/"
