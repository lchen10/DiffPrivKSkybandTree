package kSpacePartition.boxplot;
...
public class NormalDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node.currentID = 1;
				Node root = p.partitionByOptimalKNoisyNegative(k, "dataset", "normal10k.csv", level, true, epslevel,
						eps * (delta - trate), ",", dw, dh, Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("boxplot/noisynegative/normal/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("boxplot/noisynegative/normal/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "boxplot/noisynegative/normal", "boxplot/noisynegative/normal",
						new Comparison[] { Comparison.MIN, Comparison.MIN }, delta, null);
	...
	public void testLoadPoints() throws Exception {
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("boxplot/noisynegative/normal-geometric" + "-eps-" + eps + ".csv");
			out.println(
			...
			PrintWriter outdetail = new PrintWriter("boxplot/noisynegative/normal-detail" + "-eps-" + eps + ".csv");
			outdetail.println(
					"k,eps,minsize, kdetal, max level, split rate, tree rate, removal level, true coutn, private count,tp,fp,fn,precision,recall,fmeasure,totalerror,avgerror");
			PrintWriter outbox = new PrintWriter("boxplot/noisynegative/normal-boxplotF1" + "-eps-" + eps + ".csv");
			String delimit = "";
			for (int k : ks) {
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/" + "normal10k.csv" + "-BNL-k-" + k + ".csv";
											String privatefile = "boxplot/noisynegative/normal/"
													+ "dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level
													+ "-r-" + srate + "-t-" + trate + "-rl-" + removalLevel + "-min-"
													+ minsize + "-kdelta-" + kdelta + "-eps-" + eps + "." + o + ".csv";
											...
											outdetail.println(k + "," + eps + "," + minsize + "," + kdelta + "," + level + ","
													+ srate + "," + trate + "," + removalLevel + "," + trueset.size()
													+ "," + privateset.size() + "," + tp + "," + fp + ","
													+ fn + "," + precision + "," + recall + ","
													+ fmeasure + "," + totalerror + "," + avgerror);
