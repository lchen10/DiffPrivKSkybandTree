...
public class DiffPrivOptimalKSpaceNSPartitionTest {
	@Test
	public void testLoadPoints() throws Exception {
		...
		double[] treerates = new double[] { 0.7,0.8,0.9 };
		int ite = 1;
					...
					for (int x = 0; x < ite; x++) {
						for (double eps : epslist) {
							...
							for (int i = 0; i < epslevel.length; i++) {
								epslevel[i] = (eps * trate) / level;
							}
							Node root = p.partitionByOptimalKNS(k, "15-clusters.dat", level, true, epslevel,
									eps * (1 - trate));
							PrintWriter out = new PrintWriter("kspaceoutput/dfoptimalKout.txt");
							PrintWriter grid = new PrintWriter("kspaceoutput/dfoptimalKgrid-ns-k-" + k + "-level-"
