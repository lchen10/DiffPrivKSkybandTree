...
public class KDHybridBBSDiffPrivacyTest {
	@Test
	public void testBBSDiffPrivacy() throws Exception {
		...
		for (int k = 100; k <= 100; k += 20) {
			for (double eps : epslist) {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("kdhybrid-15-clusters-eps-"+eps+".txt",
						"kdhybrid-15-clusters-bbs-skyband-eps-"+eps+".csv", k, "privkd_bbs_output", "grids", new Comparison[] { Comparison.MIN, Comparison.MIN }, 1, null);
