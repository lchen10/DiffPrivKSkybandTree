...
public class KDHybridFMeasureTest {
	@Test
	public void testFMeasure() throws Exception {
		double[] epslist = new double[] { 0.1, 0.5, 1.0, 2.0 };
		PrintWriter out = new PrintWriter("fmeasure/kdhybrid.csv");
		...
		for (int k = 100; k <= 100; k += 20) {
			for (double eps : epslist) {
				...
				String privatefile = "privkd_bbs_output/" + "kdhybrid-15-clusters-bbs-skyband-eps-" + eps + ".csv-k-"
						+ k + ".csv";
				...
				HashSet<IDTuple> trueset = new HashSet<>();
				for (int i = 0; i < truelines.size(); i++) {
					String[] line = truelines.get(i).split(",");
					double x = Double.parseDouble(line[0]);
					double y = Double.parseDouble(line[1]);
					trueset.add(new IDTuple(new Double[] { x, y }, i));
				}
