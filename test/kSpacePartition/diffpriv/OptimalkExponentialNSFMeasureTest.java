...
public class OptimalkExponentialNSFMeasureTest {
	@Test
	public void testFMeasure() throws Exception {
		double[] epslist = new double[] { 2, 4, 100 };
		double[] srates = new double[] { 0.05 };
		int[] levels = new int[] { 6,7,8 };
		PrintWriter out = new PrintWriter("fmeasure/optimalK-ns-exp.csv");
		int ite = 1;
		out.println("k,rate,level,eps,true coutn, private count,tp,fp,fn,precision,recall,fmeasure,totalerror,avgerror");
					...
					for (double eps : epslist) {
						...
						for (int o = 0; o < ite; o++) {
							...
							String privatefile = "privkspace_bbs_output/"
									+ "dfoptimalKgrid-ns-exp-bbs-skyband-k-"
									+ k + "-level-" + level + "-r-" + srate
									+ "-eps-" + eps + "." + o + ".csv";
						...
						out.println(k + ","+srate+"," + level + "," + eps + ","
								+ t_truesize / ite + "," + t_privsize / ite
								+ "," + t_tp / ite + "," + t_fp / ite + ","
								+ t_fn / ite + "," + t_precision / ite + ","
								+ t_recall / ite + "," + t_fmeasure / ite + ","
