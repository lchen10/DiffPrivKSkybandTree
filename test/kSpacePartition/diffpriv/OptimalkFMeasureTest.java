...
public class OptimalkFMeasureTest {
	@Test
	public void testFMeasure() throws Exception {
		double[] epslist = new double[] { 0.1, 0.5, 1.0, 2.0 };
		PrintWriter out = new PrintWriter("fmeasure/optimalK.csv");
		int ite = 1;
		...
		for (int k = 100; k <= 100; k += 20) {
			for (double eps : epslist) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "privkspace_bbs_output/" + "dfoptimalKgrid-bbs-skyband-k-" + k + "-eps-" + eps
							+ "." + o + ".csv-k-" + k + ".csv";
