...
import kSpacePartition.diffpriv.adaptivegrid.AdaptiveGrid15ClustersDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import skyband.BBSDiffPrivacy;
import skyband.Comparison;
import skyband.Tuple;

public class AdaptiveGridAntiDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("anti10k-eps-" + eps + ".txt",
						"dfoptimalKgrid-anti10k-bbs-skyband-k-" + k + "-eps-" + eps + "." + ite + ".csv", k,
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter("fmeasure/ad-anti-noisynegativelevelremoval.csv");
		...
		for (int k : ks) {
			...
			for (double eps : epslist) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "adaptivegrid_output/" + "dfoptimalKgrid-anti10k-bbs-skyband-k-" + k + "-eps-" + eps
							+ "." + o + ".csv";
