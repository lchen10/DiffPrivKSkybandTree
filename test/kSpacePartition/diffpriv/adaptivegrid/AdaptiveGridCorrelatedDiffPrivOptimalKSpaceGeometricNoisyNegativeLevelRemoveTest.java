...
import kSpacePartition.diffpriv.adaptivegrid.AdaptiveGridAntiDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import skyband.BBSDiffPrivacy;
import skyband.Comparison;
import skyband.Tuple;

public class AdaptiveGridCorrelatedDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("correlated10k-eps-" + eps + ".txt",
						"dfoptimalKgrid-corr-bbs-skyband-k-" + k + "-eps-" + eps + "." + ite + ".csv", k,
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter("fmeasure/ad-corr-noisynegativelevelremoval.csv");
		...
		for (int k : ks) {
			...
			for (double eps : epslist) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "adaptivegrid_output/" + "dfoptimalKgrid-corr-bbs-skyband-k-" + k + "-eps-" + eps
							+ "." + o + ".csv";
