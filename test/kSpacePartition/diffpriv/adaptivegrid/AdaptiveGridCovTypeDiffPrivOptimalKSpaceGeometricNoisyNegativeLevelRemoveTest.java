...
import kSpacePartition.diffpriv.adaptivegrid.AdaptiveGridCorrelatedDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import skyband.BBSDiffPrivacy;
import skyband.Comparison;
import skyband.Tuple;

public class AdaptiveGridCovTypeDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("covtype-eps-" + eps + ".txt",
						"dfoptimalKgrid-covtype-bbs-skyband-k-" + k + "-eps-" + eps + "." + ite + ".csv", k,
						"adaptivegrid_output", "adaptiveGrid", new Comparison[] { Comparison.MAX, Comparison.MAX },
						delta, null);
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter("fmeasure/ad-covtype-noisynegativelevelremoval.csv");
		...
		for (int k : ks) {
			...
			for (double eps : epslist) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "adaptivegrid_output/" + "dfoptimalKgrid-covtype-bbs-skyband-k-" + k + "-eps-" + eps
							+ "." + o + ".csv";
				...
				out.println(k + "," + eps + "," + 
						 + t_truesize / ite + "," + t_privsize / ite + "," + t_tp / ite + "," + t_fp / ite + ","
						+ t_fn / ite + "," + t_precision / ite + "," + t_recall / ite + "," + t_fmeasure / ite + ","
