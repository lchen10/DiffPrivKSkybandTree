...
import kSpacePartition.diffpriv.adaptivegrid.AdaptiveGridCovTypeDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import skyband.BBSDiffPrivacy;
import skyband.Comparison;
import skyband.Tuple;

public class AdaptiveGridNBADiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				System.out.println(ite + "th computing for k " + k + " eps "
						+ eps);
				BBSDiffPrivacy.computeBBSWithSynthesis("nba-eps-" + eps
						+ ".txt", "dfoptimalKgrid-nba-bbs-skyband-k-" + k
						+ "-eps-" + eps + "." + ite + ".csv", k,
						"adaptivegrid_output", "adaptiveGrid",
						new Comparison[] { Comparison.MAX, Comparison.MAX },
						delta, null);
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter(
				"fmeasure/ad-nba-noisynegativelevelremoval.csv");
		...
		for (int k : ks) {
			...
			for (double eps : epslist) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String truefile = "dataset/" + "nba.csv" + "-BNL-k-" + k
							+ ".csv";
					String privatefile = "adaptivegrid_output/"
							+ "dfoptimalKgrid-nba-bbs-skyband-k-" + k + "-eps-"
							+ eps + "." + o + ".csv";
