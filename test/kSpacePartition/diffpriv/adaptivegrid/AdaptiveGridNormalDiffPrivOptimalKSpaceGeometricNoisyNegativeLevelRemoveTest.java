...
import kSpacePartition.diffpriv.adaptivegrid.AdaptiveGridNBADiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import skyband.BBSDiffPrivacy;
import skyband.Comparison;
import skyband.Tuple;

public class AdaptiveGridNormalDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("normal10k-eps-" + eps + ".txt",
						"dfoptimalKgrid-normal-bbs-skyband-k-" + k + "-eps-" + eps + "." + ite + ".csv", k,
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter("fmeasure/ad-normal-noisynegativelevelremoval.csv");
		...
		for (int k : ks) {
			...
			for (double eps : epslist) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "adaptivegrid_output/" + "dfoptimalKgrid-normal-bbs-skyband-k-" + k + "-eps-" + eps
							+ "." + o + ".csv";
