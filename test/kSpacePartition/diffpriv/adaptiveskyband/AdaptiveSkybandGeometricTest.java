...
public class AdaptiveSkybandGeometricTest {
	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByAdaptiveKNoisyNegative(k, "data",
				...
				PrintWriter out = new PrintWriter(
						"adaptiveskyband/15clusters/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter(
						"adaptiveskyband/15clusters/dfoptimalKgrid-nglr-k-" + k
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("dfoptimalKgrid-nglr-k-"
						...
						+ "-rl-" + removalLevel + "-min-" + minsize
						+ "-kdelta-" + kdelta + "-eps-" + eps + "." + ite
						+ ".csv", k, "adaptiveskyband/15clusters", "adaptiveskyband/15clusters",
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter(
				"fmeasure/as-15clusters-noisynegativelevelremoval-geometric.csv");
									...
									for (double eps : epslist) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "data/"
													+ "15-clusters.dat"
													+ "-BNL-k-" + k + ".csv";
											String privatefile = "adaptiveskyband/15clusters/"
