package kSpacePartition.diffpriv.kadaptivegrid;
...
public class AdaptiveSkybandGeometricTest {
	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByAdaptiveUniformGridNoisyNegative(k, "data",
						"15-clusters.dat", level, true, eps, "\\s+", dw, dh,
						Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter(
						"Kadaptivegrid/15clusters/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter(
						"Kadaptivegrid/15clusters/dfoptimalKgrid-nglr-k-" + k
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("dfoptimalKgrid-nglr-k-"
						...
						+ "-rl-" + removalLevel + "-min-" + minsize
						+ "-kdelta-" + kdelta + "-eps-" + eps + "." + ite
						+ ".csv", k, "Kadaptivegrid/15clusters", "Kadaptivegrid/15clusters",
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter(
				"fmeasure/Kadaptivegrid/kad-15clusters-noisynegativelevelremoval-geometric.csv");
									...
									for (double eps : epslist) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "data/"
													+ "15-clusters.dat"
													+ "-BNL-k-" + k + ".csv";
											String privatefile = "Kadaptivegrid/15clusters/"
