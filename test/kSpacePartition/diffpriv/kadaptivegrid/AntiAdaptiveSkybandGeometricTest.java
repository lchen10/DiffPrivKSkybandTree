...
public class AntiAdaptiveSkybandGeometricTest {
	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByAdaptiveUniformGridNoisyNegative(k, "dataset",
						"anti10k.csv", level, true, eps, ",", dw, dh,
						Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter(
						"Kadaptivegrid/anti/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter(
						"Kadaptivegrid/anti/dfoptimalKgrid-nglr-k-" + k
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("dfoptimalKgrid-nglr-k-"
						...
						+ "-rl-" + removalLevel + "-min-" + minsize
						+ "-kdelta-" + kdelta + "-eps-" + eps + "." + ite
						+ ".csv", k, "Kadaptivegrid/anti", "Kadaptivegrid/anti",
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter(
				"fmeasure/Kadaptivegrid/kad-anti-noisynegativelevelremoval-geometric.csv");
									...
									for (double eps : epslist) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/"
													+ "anti10k.csv"
													+ "-BNL-k-" + k + ".csv";
											String privatefile = "Kadaptivegrid/anti/"
