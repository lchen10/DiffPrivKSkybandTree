...
public class CorrAdaptiveSkybandGeometricTest {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByAdaptiveKNoisyNegative(k, "dataset",
						"correlated10k.csv", level, true, eps, ",", dw, dh,
						Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter(
						"adaptiveskyband/corr/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter(
						"adaptiveskyband/corr/dfoptimalKgrid-nglr-k-" + k
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("dfoptimalKgrid-nglr-k-"
						...
						+ "-rl-" + removalLevel + "-min-" + minsize
						+ "-kdelta-" + kdelta + "-eps-" + eps + "." + ite
						+ ".csv", k, "adaptiveskyband/corr", "adaptiveskyband/corr",
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter(
				"fmeasure/corr/as-corr-noisynegativelevelremoval-geometric.csv");
									...
									for (double eps : epslist) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/"
													+ "correlated10k.csv"
													+ "-BNL-k-" + k + ".csv";
											String privatefile = "adaptiveskyband/corr/"
