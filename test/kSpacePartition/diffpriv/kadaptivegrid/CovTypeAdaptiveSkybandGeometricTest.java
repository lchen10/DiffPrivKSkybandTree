...
public class CovTypeAdaptiveSkybandGeometricTest {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByAdaptiveKNoisyNegative(k, "dataset",
						"covtype.csv", level, true, eps, ",", dw, dh,
						Comparison.MAX, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter(
						"adaptiveskyband/covtype/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter(
						"adaptiveskyband/covtype/dfoptimalKgrid-nglr-k-" + k
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("dfoptimalKgrid-nglr-k-"
						...
						+ "-rl-" + removalLevel + "-min-" + minsize
						+ "-kdelta-" + kdelta + "-eps-" + eps + "." + ite
						+ ".csv", k, "adaptiveskyband/covtype", "adaptiveskyband/covtype",
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter(
				"fmeasure/covtype/as-covtype-noisynegativelevelremoval-geometric.csv");
									...
									for (double eps : epslist) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/"
													+ "covtype.csv"
													+ "-BNL-k-" + k + ".csv";
											String privatefile = "adaptiveskyband/covtype/"
