...
public class NBAAdaptiveSkybandGeometricTest {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByAdaptiveKNoisyNegative(k, "dataset",
						"nba.csv", level, true, eps, ",", dw, dh,
						Comparison.MAX, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter(
						"adaptiveskyband/nba/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter(
						"adaptiveskyband/nba/dfoptimalKgrid-nglr-k-" + k
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("dfoptimalKgrid-nglr-k-"
						...
						+ "-rl-" + removalLevel + "-min-" + minsize
						+ "-kdelta-" + kdelta + "-eps-" + eps + "." + ite
						+ ".csv", k, "adaptiveskyband/nba", "adaptiveskyband/nba",
	...
	public void testLoadPoints() throws Exception {
		...
		PrintWriter out = new PrintWriter(
				"fmeasure/as-nba-noisynegativelevelremoval-geometric.csv");
									...
									for (double eps : epslist) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/"
													+ "nba.csv"
													+ "-BNL-k-" + k + ".csv";
											String privatefile = "adaptiveskyband/nba/"
