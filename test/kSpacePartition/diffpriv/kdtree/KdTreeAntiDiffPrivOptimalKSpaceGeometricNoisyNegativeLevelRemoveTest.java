...
import kSpacePartition.diffpriv.kdtree.KdTree15ClustersDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import skyband.BBSDiffPrivacy;
import skyband.Comparison;
import skyband.Tuple;

public class KdTreeAntiDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("anti10k-eps-" + eps + "." + x + ".txt",
						"dfoptimalKgrid-corr-bbs-skyband-k-" + k + "-eps-" + eps + "." + x + ".csv", k, "kdtree/anti",
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/kdtree/anti-geometric" + "-eps-" + eps + ".csv");
			...
			for (int k : ks) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "kdtree/anti/" + "dfoptimalKgrid-corr-bbs-skyband-k-" + k + "-eps-" + eps + "."
							+ o + ".csv";
