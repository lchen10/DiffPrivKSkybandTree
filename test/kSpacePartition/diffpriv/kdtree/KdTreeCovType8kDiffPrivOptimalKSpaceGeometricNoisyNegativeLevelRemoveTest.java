...
public class KdTreeCovType8kDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("covtype-8k-eps-" + eps + "." + x + ".txt",
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
						+ "-rl-" + removalLevel + "-min-" + minsize + "-kdelta-" + kdelta + "-eps-" + eps + "."
						+ x + ".csv", k,
						"kdtree/covtype-8k", "kdtree_data", new Comparison[] { Comparison.MAX, Comparison.MAX }, delta,
						null);
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/kdtree/covtype-8k-geometric" + "-eps-" + eps + ".csv");
			...
			for (int k : ks) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "kdtree/covtype-8k/" + "dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + 0 + "-r-" + 0.0 + "-t-" + 0.0
