...
public class KdTreeCovTypeDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("covtype-eps-" + eps + "." + x + ".txt",
						"dfoptimalKgrid-covtype-bbs-skyband-k-" + k + "-eps-" + eps
						+ "." + x + ".csv", k,
						"kdtree/covtype", "kdtree_data", new Comparison[] { Comparison.MAX, Comparison.MAX }, delta,
						null);
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/kdtree/covtype-geometric" + "-eps-" + eps + ".csv");
			...
			for (int k : ks) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "kdtree/covtype/" + "dfoptimalKgrid-covtype-bbs-skyband-k-" + k + "-eps-" + eps
							+ "." + o + ".csv";
