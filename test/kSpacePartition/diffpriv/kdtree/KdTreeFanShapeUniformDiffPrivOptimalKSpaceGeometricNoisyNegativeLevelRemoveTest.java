...
public class KdTreeFanShapeUniformDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("fan-uniform-eps-" + eps + "." + x + ".txt",
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
						+ "-rl-" + removalLevel + "-min-" + minsize + "-kdelta-" + kdelta + "-eps-" + eps + "."
						+ x + ".csv", k,
						"kdtree/fan-uniform", "kdtree_data", new Comparison[] { Comparison.MAX, Comparison.MAX }, delta,
						null);
	...
	public void testLoadPoints() throws Exception {
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/kdtree/fan-uniform-geometric" + "-eps-" + eps + ".csv");
			...
			for (int k : ks) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "kdtree/fan-uniform/" + "dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + 0 + "-r-" + 0.0 + "-t-" + 0.0
