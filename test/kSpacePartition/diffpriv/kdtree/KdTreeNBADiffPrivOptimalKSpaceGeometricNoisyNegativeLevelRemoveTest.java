...
import kSpacePartition.diffpriv.kdtree.KdTreeCovTypeDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import skyband.BBSDiffPrivacy;
import skyband.Comparison;
import skyband.Tuple;

public class KdTreeNBADiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("nba-eps-" + eps + "." + x + ".txt",
						"dfoptimalKgrid-nba-bbs-skyband-k-" + k + "-eps-" + eps + "." + x + ".csv", k, "kdtree/nba",
						"kdtree_data", new Comparison[] { Comparison.MAX, Comparison.MAX }, delta, null);
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/kdtree/nba-geometric" + "-eps-" + eps + ".csv");
			...
			for (int k : ks) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "kdtree/nba/" + "dfoptimalKgrid-nba-bbs-skyband-k-" + k + "-eps-" + eps + "."
							+ o + ".csv";
