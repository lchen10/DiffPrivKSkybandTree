...
import kSpacePartition.diffpriv.kdtree.KdTreeNBADiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import skyband.BBSDiffPrivacy;
import skyband.Comparison;
import skyband.Tuple;

public class KdTreeNormalDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class BBSThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis("normal10k-eps-" + eps + "." + x + ".txt",
						"dfoptimalKgrid-normal-bbs-skyband-k-" + k + "-eps-" + eps
						+ "." + x + ".csv",
						k, "kdtree/normal", "kdtree_data", new Comparison[] { Comparison.MIN, Comparison.MIN }, delta,
						null);
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/kdtree/normal-geometric" + "-eps-" + eps + ".csv");
			...
			for (int k : ks) {
				...
				for (int o = 0; o < ite; o++) {
					...
					String privatefile = "kdtree/normal/" + "dfoptimalKgrid-normal-bbs-skyband-k-" + k + "-eps-" + eps
							+ "." + o + ".csv";
