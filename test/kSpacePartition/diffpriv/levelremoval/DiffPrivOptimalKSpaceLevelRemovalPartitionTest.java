...
public class DiffPrivOptimalKSpaceLevelRemovalPartitionTest {
	@Test
	public void testLoadPoints() throws Exception {
						...
						for (int x = 0; x < ite; x++) {
							for (double eps : epslist) {
								...
								PrintWriter grid = new PrintWriter(
										"kspaceoutput/dfoptimalKgrid-lr-k-" + k
												+ "-level-" + level + "-r-"
												+ srate + "-t-" + trate
												+ "-rl-" + removalLevel
												+ "-eps-" + eps + "." + x
							...
							for (k = 100; k <= 100; k += 20) {
								for (double eps : epslist) {
									...
									BBSDiffPrivacy.computeBBSWithSynthesis(
											"dfoptimalKgrid-lr-k-" + k
													+ "-level-" + level + "-r-"
													+ srate + "-t-" + trate
													+ "-rl-" + removalLevel
													+ "-eps-" + eps + "." + x
													+ ".csv",
											"dfoptimalKgrid-lr-bbs-skyband-k-"
													+ k + "-level-" + level
													+ "-r-" + srate + "-t-"
													+ trate + "-rl-"
													+ removalLevel + "-eps-"
		...
		PrintWriter out = new PrintWriter(
				"fmeasure/optimalK-levelremoval-uniform.csv");
					...
					for (double srate : splitrates) {
						for (int level : levels) {
							for (double eps : epslist) {
								...
								out.println(k + "," + eps + "," + level + ","
										+ srate + "," + trate + ","
										+ removalLevel + "," + t_truesize / ite
										+ "," + t_privsize / ite + ","
										+ t_tp / ite + "," + t_fp / ite + ","
										+ t_fn / ite + "," + t_precision / ite
										+ "," + t_recall / ite + ","
		...
		PrintWriter out2 = new PrintWriter(
				"fmeasure/optimalK-levelremoval-uniform-grid.csv");
		out2.println("k,eps, max level, split rate, tree rate, removal level, true coutn, private count,tp,fp,fn,precision,recall,fmeasure,totalerror,avgerror");
							...
							for (double eps : epslist) {
								...
								for (int o = 0; o < ite; o++) {
									...
									for (int i = 0; i < truelines.size(); i++) {
										...
										trueset.add(new Tuple(new int[] { xi,
												yi }));
									}
									...
									for (int i = 0; i < privatelines.size(); i++) {
										...
										int xi = (int) (x / winterval) + 1;
										int yi = (int) (y / hinterval) + 1;
										privateset.add(new Tuple(new int[] {
												xi, yi }));
									}
