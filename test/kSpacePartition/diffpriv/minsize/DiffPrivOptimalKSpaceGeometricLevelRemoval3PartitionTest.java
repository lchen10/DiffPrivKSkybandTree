...
public class DiffPrivOptimalKSpaceGeometricLevelRemoval3PartitionTest {
	@Test
...
//								}
								...
								PrintWriter grid = new PrintWriter(
										"kspaceoutput/dfoptimalKgrid-glr3-k-" + k
												+ "-level-" + level + "-r-"
												+ srate + "-t-" + trate
												+ "-rl-" + removalLevel
												+ "-eps-" + eps + "." + x
							...
							for (k = 100; k <= 100; k += 20) {
								for (double eps : epslist) {
									...
									BBSDiffPrivacy.computeBBSWithSynthesis(
											"dfoptimalKgrid-glr3-k-" + k
													+ "-level-" + level + "-r-"
													+ srate + "-t-" + trate
													+ "-rl-" + removalLevel
													+ "-eps-" + eps + "." + x
													+ ".csv",
											"dfoptimalKgrid-glr3-bbs-skyband-k-"
													+ k + "-level-" + level
													+ "-r-" + srate + "-t-"
													+ trate + "-rl-"
													+ removalLevel + "-eps-"
		...
		PrintWriter out = new PrintWriter("fmeasure/optimalK-geometriclevel3removal.csv");
