...
public class DiffPrivOptimalKSpaceMinSizeAdjustPartitionTest {
	@Test
					...
					for (int x = 0; x < ite; x++) {
						for (double eps : epslist) {
							...
							Node root = p.partitionByOptimalKMinSize(k, "15-clusters.dat", level, true, epslevel,
									eps * (1 - trate));
							root = p.adjustConsistency(root, epslevel, level);
							PrintWriter out = new PrintWriter("kspaceoutput/dfoptimalKout.txt");
							PrintWriter grid = new PrintWriter("kspaceoutput/dfoptimalKgrid-msa-k-" + k + "-level-"
