	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				MultiDimNode root = p.partitionByOptimalKNoisyNegative(k, "dataset", "anti3d.csv", level, true,
						epslevel, eps * (delta - trate), ",", rangewidth, Comparison.MAX, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("multidim/anti/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("multidim/anti/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
...
//		 double[] epslist = new double[] { 2 };
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/multidim/anti-geometric" + "-eps-" + eps + ".csv");
			out.println(
			...
			PrintWriter outdetail = new PrintWriter("fmeasure/multidim/anti-detail" + "-eps-" + eps + ".csv");
			outdetail.println(
					"k,eps,minsize, kdetal, max level, split rate, tree rate, removal level, true coutn, private count,tp,fp,fn,precision,recall,fmeasure,totalerror,avgerror");
			PrintWriter outbox = new PrintWriter("fmeasure/multidim/anti-boxplotF1" + "-eps-" + eps + ".csv");
			String delimit = "";
			for (int k : ks) {
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/" + "anti3d.csv" + "-BNL-k-" + k + ".csv";
											String privatefile = "multidim/anti/" + "dfoptimalKgrid-nglr-bbs-skyband-k-"
