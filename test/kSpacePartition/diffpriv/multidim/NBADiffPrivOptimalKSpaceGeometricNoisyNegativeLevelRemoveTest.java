...
public class NBADiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				MultiDimNode root = p.partitionByOptimalKNoisyNegative(k,
						"dataset", "nba3d.csv", level, true, epslevel, eps
								* (delta - trate), ",", rangewidth,
						Comparison.MAX, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter(
						"multidim/nba/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter(
						"multidim/nba/dfoptimalKgrid-nglr-k-" + k + "-level-"
								+ level + "-r-" + srate + "-t-" + trate
								+ "-rl-" + removalLevel + "-min-" + minsize
								+ "-kdelta-" + kdelta + "-eps-" + eps + "."
								+ ite + ".csv");
				...
				while (!q.isEmpty()) {
					...
					String postix = childrencount + "," + n.id + ","
		...
		public void run() {
			...
			try {
				...
				MultiDimBBSDiffPrivacy.computeBBSWithSynthesis(dim,
						"dfoptimalKgrid-nglr-k-" + k + "-level-" + level
								+ "-r-" + srate + "-t-" + trate + "-rl-"
								+ removalLevel + "-min-" + minsize + "-kdelta-"
								+ kdelta + "-eps-" + eps + "." + ite + ".csv",
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-"
								+ level + "-r-" + srate + "-t-" + trate
								+ "-rl-" + removalLevel + "-min-" + minsize
								+ "-kdelta-" + kdelta + "-eps-" + eps + "."
								+ ite + ".csv", k, "multidim/nba",
						"multidim/nba", new Comparison[] { Comparison.MAX,
								Comparison.MAX, Comparison.MAX }, delta, null);
	...
	public void testLoadPoints() throws Exception {
		...
		MultiParam.MINS[1] = 0;
		MultiParam.MINS[2] = 0;
		MultiParam.MAXS[0] = 3156 + 1;
		MultiParam.MAXS[1] = 1449 + 1;
		MultiParam.MAXS[2] = 1060 + 1;
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/multidim/nba-geometric"
					+ "-eps-" + eps + ".csv");
			...
			PrintWriter outdetail = new PrintWriter(
					"fmeasure/multidim/nba-detail" + "-eps-" + eps + ".csv");
			outdetail
					.println("k,eps,minsize, kdetal, max level, split rate, tree rate, removal level, true coutn, private count,tp,fp,fn,precision,recall,fmeasure,totalerror,avgerror");
			PrintWriter outbox = new PrintWriter(
					"fmeasure/multidim/nba-boxplotF1" + "-eps-" + eps + ".csv");
			String delimit = "";
			for (int k : ks) {
									...
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/"
													+ "nba3d.csv" + "-BNL-k-"
													+ k + ".csv";
											String privatefile = "multidim/nba/"
