...
public class NormalDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				MultiDimNode root = p.partitionByOptimalKNoisyNegative(k, "dataset", "normal3d.csv", level, true,
						epslevel, eps * (delta - trate), ",", rangewidth, Comparison.MAX, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("multidim/normal/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("multidim/normal/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				MultiDimBBSDiffPrivacy.computeBBSWithSynthesis(dim,
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "multidim/normal", "multidim/normal",
	...
	public void testLoadPoints() throws Exception {
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/multidim/normal-geometric" + "-eps-" + eps + ".csv");
			out.println(
			...
			PrintWriter outdetail = new PrintWriter("fmeasure/multidim/normal-detail" + "-eps-" + eps + ".csv");
			outdetail.println(
					"k,eps,minsize, kdetal, max level, split rate, tree rate, removal level, true coutn, private count,tp,fp,fn,precision,recall,fmeasure,totalerror,avgerror");
			PrintWriter outbox = new PrintWriter("fmeasure/multidim/normal-boxplotF1" + "-eps-" + eps + ".csv");
			String delimit = "";
			for (int k : ks) {
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String truefile = "dataset/" + "normal3d.csv" + "-BNL-k-" + k + ".csv";
											String privatefile = "multidim/normal/"
