...
public class MergeResultTest {
	...
	public void testMergeFmeasure() throws Exception {
		...
		// "quadtree" };
		String[] folders = new String[] { "multidim100k" };
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("evaluations/multidim100k/" + name + "/results-eps-" + eps + ".csv");
			for (String dataset : datasets) {
				...
				for (String folder : folders) {
					List<String> lines = Files.readAllLines(
							Paths.get("fmeasure/" + folder + "/" + dataset + "-geometric-eps-" + eps + ".csv"),
