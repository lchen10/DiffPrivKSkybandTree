...
public class AntiDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("noisynegative/anti/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("noisynegative/anti/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "noisynegative/anti", "noisynegative/anti",
	...
	public void testLoadPoints() throws Exception {
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/noisynegative/anti-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "noisynegative/anti/"
