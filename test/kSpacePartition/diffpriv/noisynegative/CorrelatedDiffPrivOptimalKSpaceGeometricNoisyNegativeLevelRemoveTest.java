...
public class CorrelatedDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByOptimalKNoisyNegative(k, "dataset", "correlated10k.csv", level, true, epslevel,
						eps * (delta - trate), ",", dw, dh, Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("noisynegative/corr/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("noisynegative/corr/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "noisynegative/corr", "noisynegative/corr",
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/noisynegative/corr-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "noisynegative/corr/"
