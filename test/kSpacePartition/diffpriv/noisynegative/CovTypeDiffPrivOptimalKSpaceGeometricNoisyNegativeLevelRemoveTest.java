	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("noisynegative/covtype/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("noisynegative/covtype/dfoptimalKgrid-nglr-k-" + k + "-level-"
						+ level + "-r-" + srate + "-t-" + trate + "-rl-" + removalLevel + "-min-" + minsize + "-kdelta-"
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "noisynegative/covtype", "noisynegative/covtype",
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/noisynegative/covtype-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "noisynegative/covtype/"
