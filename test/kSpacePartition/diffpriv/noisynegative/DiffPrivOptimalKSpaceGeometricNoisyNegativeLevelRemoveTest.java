...
public class DiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node.currentID = 1;
				Node root = p.partitionByOptimalKNoisyNegative(k, "data", "15-clusters.dat", level, true, epslevel,
						eps * (1 - trate), "\\s+", dw, dh, Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("noisynegative/15clusters/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("noisynegative/15clusters/dfoptimalKgrid-nglr-k-" + k + "-level-"
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "noisynegative/15clusters", "noisynegative/15clusters",
						new Comparison[] { Comparison.MIN, Comparison.MIN }, delta, perfs);
	...
	public void testLoadPoints() throws Exception {
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/noisynegative/15clusters-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "noisynegative/15clusters/"
