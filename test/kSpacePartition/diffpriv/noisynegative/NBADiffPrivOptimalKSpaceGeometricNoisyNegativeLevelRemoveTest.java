	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("noisynegative/nba/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("noisynegative/nba/dfoptimalKgrid-nglr-k-" + k + "-level-"
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "noisynegative/nba", "noisynegative/nba",
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/noisynegative/nba-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "noisynegative/nba/"
