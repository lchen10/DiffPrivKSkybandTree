package kSpacePartition.diffpriv.noisynegative;
...
import kSpacePartition.diffpriv.noisynegative.DiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.BBSThread;
import kSpacePartition.diffpriv.noisynegative.DiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest.SpaceSplitThread;
...
public class NormalDiffPrivOptimalKSpaceGeometricNoisyNegativeLevelRemoveTest {
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				double[] epslevel = new double[level + delta];
				...
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("noisynegative/normal/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("noisynegative/normal/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "noisynegative/normal", "noisynegative/normal",
...
//		}
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/noisynegative/normal-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "noisynegative/normal/"
