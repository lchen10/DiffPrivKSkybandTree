package kSpacePartition.diffpriv.quadtree;
...
public class QuadTree15ClustersGeometricTest {
	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByQuadtree(k, "data", "15-clusters.dat", level, true, epslevel,
						eps * (1 - trate), "\\s+", dw, dh, Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("quadtree/15clusters/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("quadtree/15clusters/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "quadtree/15clusters", "quadtree/15clusters",
...
//		perfout.close();
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/quadtree/15clusters-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "quadtree/15clusters/"
											...
											for (Tuple tuple : privateset) {
												...
												for (Tuple truetuple : trueset) {
													double d = Math.abs(truetuple.getValue(0) - x)
															+ Math.abs(truetuple.getValue(delta) - y);
													if (mind > d) {
														mind = d;
													}
												}
