...
public class QuadTreeAntiGeometricTest {
	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByQuadtree(k, "dataset", "anti10k.csv", level, true, epslevel, eps * (1 - trate),
						",", dw, dh, Comparison.MIN, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("quadtree/anti/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("quadtree/anti/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "quadtree/anti", "quadtree/anti", new Comparison[] { Comparison.MIN, Comparison.MIN }, delta,
						perfs);
...
//		}
		...
		perfout.close();
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/quadtree/anti-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "quadtree/anti/" + "dfoptimalKgrid-nglr-bbs-skyband-k-"
