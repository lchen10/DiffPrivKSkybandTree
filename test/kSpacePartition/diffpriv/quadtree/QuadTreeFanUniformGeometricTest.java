...
public class QuadTreeFanUniformGeometricTest {
	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node.currentID = 1;
				Node root = p.partitionByQuadtree(k, "dataset", "fan-uniform.csv", level, true, epslevel, eps * (1 - trate),
						",", dw, dh, Comparison.MAX, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("quadtree/fan-uniform/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("quadtree/fan-uniform/dfoptimalKgrid-nglr-k-" + k + "-level-" + level
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "quadtree/fan-uniform", "quadtree/fan-uniform", new Comparison[] { Comparison.MAX, Comparison.MAX },
						delta, perfs);
	...
	public void testLoadPoints() throws Exception {
		...
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/quadtree/fan-uniform-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "quadtree/fan-uniform/"
