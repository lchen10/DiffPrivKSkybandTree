...
public class QuadTreeNBAGeometricTest {
	...
	class SpaceSplitThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Node root = p.partitionByQuadtree(k, "dataset", "nba.csv", level, true, epslevel, eps * (1 - trate),
						",", dw, dh, Comparison.MAX, minsize, kdelta);
				// root = p.postProcessing(root,
				// epslevel,
				// level);
				PrintWriter out = new PrintWriter("quadtree/nba/dfoptimalKout.txt");
				PrintWriter grid = new PrintWriter("quadtree/nba/dfoptimalKgrid-nglr-k-" + k + "-level-" + level + "-r-"
						+ srate + "-t-" + trate + "-rl-" + removalLevel + "-min-" + minsize + "-kdelta-" + kdelta
						+ "-eps-" + eps + "." + ite + ".csv");
			...
			try {
				...
				BBSDiffPrivacy.computeBBSWithSynthesis(
						...
						"dfoptimalKgrid-nglr-bbs-skyband-k-" + k + "-level-" + level + "-r-" + srate + "-t-" + trate
								...
								+ ite + ".csv",
						k, "quadtree/nba", "quadtree/nba", new Comparison[] { Comparison.MAX, Comparison.MAX }, delta,
						perfs);
...
//		}
		...
		perfout.close();
		for (double eps : epslist) {
			PrintWriter out = new PrintWriter("fmeasure/quadtree/nba-geometric" + "-eps-" + eps + ".csv");
			out.println(
								...
								for (double srate : splitrates) {
									for (int level : levels) {
										...
										for (int o = 0; o < ite; o++) {
											...
											String privatefile = "quadtree/nba/" + "dfoptimalKgrid-nglr-bbs-skyband-k-"
