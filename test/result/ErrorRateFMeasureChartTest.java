...
public class ErrorRateFMeasureChartTest {
	...
	public void testChart() throws Exception {
		...
		int[] ks = new int[] { 20, 40, 60, };

		double[] epslist = new double[] { 1, 2 };
		double[] errlist = new double[] { 0.01, 0.03, 0.05, 0.07, 0.1 };

		String[] datasets = new String[] { "nba", "covtype" };
		...
		for (int k : ks) {
			for (double eps : epslist) {
				for (String data : datasets) {
					...
					XYSeries kdtree = new XYSeries("KD-Tree");
					XYSeries quadtree = new XYSeries("Quad-Tree");
					int index = 0;

					for (double err : errlist) {
						List<String> noisynegativelines = Files.readAllLines(Paths
								.get("fmeasure/noisynegative/" + err + "/" + data + "-geometric-eps-" + eps + ".csv"));

						for (int i = 1; i < noisynegativelines.size(); i++) {
							String line = noisynegativelines.get(i);
							String[] split = line.split(",");
							if (k != Integer.parseInt(split[0])) {
								continue;
							}
							ksybandtree.add(index, Double.parseDouble(split[15]));
						}

						List<String> kdtreelines = Files.readAllLines(
								Paths.get("fmeasure/kdtree/" + err + "/" + data + "-geometric-eps-" + eps + ".csv"));
						for (int i = 1; i < kdtreelines.size(); i++) {
							String line = kdtreelines.get(i);
							String[] split = line.split(",");
							if (k != Integer.parseInt(split[0])) {
								continue;
							}
							kdtree.add(index, Double.parseDouble(split[9]));
						}

						List<String> quadtreelines = Files.readAllLines(
								Paths.get("fmeasure/quadtree/" + err + "/" + data + "-geometric-eps-" + eps + ".csv"));

						for (int i = 1; i < quadtreelines.size(); i++) {
							String line = quadtreelines.get(i);
							String[] split = line.split(",");
							if (k != Integer.parseInt(split[0])) {
								continue;
							}
							quadtree.add(index, Double.parseDouble(split[15]));
						}
					...
					ori.addSeries(kdtree);
					ori.addSeries(quadtree);
					JFreeChart chart = ChartFactory.createXYLineChart("", "Error Tolerance Rate", "F-1", ori, PlotOrientation.VERTICAL,
							false, true, false);
					...
					SymbolAxis axis = new SymbolAxis("Error Tolerance Rate", new String[] { "[1%x1%]", "[3%x3%]", "[5%x5%]", "[7%x7%]", "[10%x10%]" });
					axis.setTickUnit(new NumberTickUnit(1));
					axis.setAutoRange(false);
					axis.setRange(-1, errlist.length);
					...
					float scaleFactor = 2F; // use to rescale the
											// datapoints
											// smaller
					...
					saveChartAsPDF(new File("evaluations/errF1Figures/" + data + "-k-" + k + "-eps-" + eps + ".pdf"),
