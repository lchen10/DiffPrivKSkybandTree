...
public class FMeasureChartTest {
	...
	public void testChart() throws Exception {
		...
		for (double eps : epslist) {
			List<String> lines = Files.readAllLines(Paths.get("evaluations/" + name + "/results-eps-" + eps + ".csv"));
			...
			for (String line : lines) {
				...
				if (split.length > 3) {
					...
					kdtree.add(index, Double.parseDouble(split[2]));
					quadtree.add(index, Double.parseDouble(split[3]));
					index++;
				}
				...
				if (split.length == 1) {
					...
					JFreeChart chart = ChartFactory.createXYLineChart("", "k", "F-1", ori, PlotOrientation.VERTICAL,
							false, true, false);
					...
					SymbolAxis axis = new SymbolAxis("k",
							new String[] { "0", "1", "3", "5", "10", "20", "40", "60", "80", "100", "150", "200", });
					axis.setTickUnit(new NumberTickUnit(1));
					axis.setAutoRange(false);
					axis.setRange(-1, ks.length);
					...
					// File orifig = new File("evaluations/"+name+"/" + filename
					// + ".png");
					// ChartUtilities.saveChartAsPNG(orifig, chart, width,
					// height);
					...
					saveChartAsPDF(new File("evaluations/" + name + "/" + filename + ".pdf"), chart, width, height,
							new DefaultFontMapper());

					ori = new XYSeriesCollection();
					ksybandtree = new XYSeries("KSkyband-Tree");
					kdtree = new XYSeries("KD-Tree");
					quadtree = new XYSeries("Quad-Tree");
					index = 0;
				}
