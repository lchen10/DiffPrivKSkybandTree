...
public class FanFMeasureChartTest {
	...
	public void testChart() throws Exception {
		...
		for (double eps : epslist) {
			List<String> lines = Files.readAllLines(Paths
					.get("evaluations/fan-uniform/results-eps-" + eps + ".csv"));

			String ds = "";
			XYSeriesCollection ori = new XYSeriesCollection();
			XYSeries noisynegative = new XYSeries("KSkyband-Tree");
			...
			for (String line : lines) {
				...
				if (split.length > 3) {
					...
					noisynegative.add(index, Double.parseDouble(split[1]));
...
//					rangeAxis.setUpperMargin(0.5);
					...
					File orifig = new File("evaluations/fmeasure/" + filename
							+ ".png");
					...
					saveChartAsPDF(new File("evaluations/fan-uniform/fmeasure/" + filename
							+ ".pdf"), chart, width, height,
							new DefaultFontMapper());

					ori = new XYSeriesCollection();
					noisynegative = new XYSeries("KSkyband-Tree");
