...
public class NBACovtypeChartTest {
	...
	private void resizeNumberTick(ValueAxis axis) {
		...
		double interval = max / 3.0;
		axis.setRange(0, max*1.04);
...
//			}
			...
			for (String kline : trueLines) {
				...
				oriseries.add(Double.parseDouble(split[0]),Double.parseDouble(split[1]));
			}
			...
			for (int k : ks) {
				...
				for (String kline : kband) {
					String[] split = kline.split(",");
					series.add(Double.parseDouble(split[0]), Double.parseDouble(split[1]));
				}
